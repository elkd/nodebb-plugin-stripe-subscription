...
stripe.configure = function() {
    meta.settings.get('stripe-subscription', function(err, settings) {
        if (!settings.api_key) {
                return winston.warn('[stripe-subscription] API Credentials not configured');
        }
            ...
            function(err, plan) {
              if(err){
                  stripe.definePlan(plan_id,plan_name,settings.monthly,function(err,_plan){
                      if(err) return winston.warn('[stripe-subscription] error configuring plan:'+err);
                  });
              } 
...
stripe.isSubscribed = function(uid, callback) {
  ...
  db.isSortedSetMember('stripe-subscription:subscribed', uid, callback);
};
                    ...
                    function(err, customer) {
                        ...
                        API.subscriptions.create(subscrip, function(err, subscription) {
                              if(err){
                                  ...
                                  res.redirect('/stripe-subscription/success');
                              }
...
stripe.onSuccess = function(req, res, next) {
	...
	meta.settings.get('stripe-subscription', function(err, settings) {
			...
			db.sortedSetAdd('stripe-subscription:subscribed', Date.now(), uid);
			user.setUserField(uid, 'stripe-subscription:cid', customer_id);
                        user.setUserField(uid, 'stripe-subscription:sid', subscription_id);
...
stripe.cancelSubscription = function(req, res, next) {
	...
	user.getUserFields(uid, ['stripe-subscription:sid'], function(err, u_obj) {
                ...
                if(err){
                    ...
                    var sid = u_obj["stripe-subscription:sid"];
                    if (!sid) {
		...
		meta.settings.get('stripe-subscription', function(err, settings) {
			API.subscriptions.del(sid, function(err, conf) {
				...
				db.deleteObjectField('user:' + uid, 'stripe-subscription:sid');
				db.sortedSetRemove('stripe-subscription:subscribed', uid);
				if (settings['premium-group']) {
